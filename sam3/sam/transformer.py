...
class RoPEAttention(Attention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Keep the initial RoPE cache on CPU so model construction does not
        # allocate CUDA memory before the model is moved to its target device.
    ...
    def forward(
        ...
        if self.freqs_cis.shape[0] != q.shape[-2]:
            ...
            self.freqs_cis_real = self.freqs_cis.real
            self.freqs_cis_imag = self.freqs_cis.imag
        elif self.freqs_cis.device != q.device:
            self.freqs_cis = self.freqs_cis.to(q.device, non_blocking=True)
            if self.use_rope_real:
                self.freqs_cis_real = self.freqs_cis.real
                self.freqs_cis_imag = self.freqs_cis.imag
