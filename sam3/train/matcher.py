...
class BinaryHungarianMatcherV2(nn.Module):
    ...
    def forward(
        ...
        # Some samples can still produce NaN/Inf costs (typically from degenerate
        # predicted boxes). Matching should treat them as invalid, not crash.
        if not torch.isfinite(C).all():
            C = torch.where(
                torch.isfinite(C),
                C,
                torch.full_like(C, 1e9),
            )
        C = C.cpu().numpy()
