...
FINITE_CHECK_PATTERNS = (
    "detector.backbone.language_backbone.*",
    "tracker.backbone.language_backbone.*",
)
...
def _describe_non_finite_tensor(name, tensor):
    finite_mask = torch.isfinite(tensor)
    bad_mask = ~finite_mask
    bad_count = int(bad_mask.sum().item())
    nan_count = int(torch.isnan(tensor).sum().item())
    posinf_count = int((tensor == float("inf")).sum().item())
    neginf_count = int((tensor == float("-inf")).sum().item())
    finite_vals = tensor.detach()[finite_mask]
    if finite_vals.numel() > 0:
        finite_abs_max = float(finite_vals.float().abs().max().item())
    else:
        finite_abs_max = None
    return (
        f"{name} shape={tuple(tensor.shape)} bad_count={bad_count} "
        f"nan={nan_count} +inf={posinf_count} -inf={neginf_count} "
        f"finite_abs_max={finite_abs_max}"
    )


def _non_finite_tensor_summaries(named_tensors, patterns=None, limit=8):
    summaries = []
    for name, tensor in named_tensors:
        ...
        if not bool(finite_mask.all()):
            summaries.append(_describe_non_finite_tensor(name, tensor))
...
class Trainer:
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        # In validation mode, skip loading optimizer and loss states
        if self.mode != "val":
            ...
            # In validation mode, only set epoch and steps to 0
            logging.info("Validation mode: skipping optimizer and loss state loading")
            self.epoch = 0
    ...
    def _step(
        ...
        # Track mean object confidence (sigmoid of presence_logit) across all stage outputs.
        # This gives a per-step signal of how confident the model is that objects exist,
        # independent of loss magnitude — useful for diagnosing confidence collapse.
        if isinstance(find_stages, SAM3Output):
            conf_vals = []
            for stage_out_list in find_stages.output:
                for out in stage_out_list:
                    pl = out.get("presence_logit")
                    if pl is None:
                        pl = out.get("presence_logit_dec")
                    if pl is not None:
                        conf_vals.append(torch.sigmoid(pl.detach().float()).mean())
            if conf_vals:
                step_losses[f"Confidence/{phase}_mean_obj_confidence"] = torch.stack(conf_vals).mean()
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                # Detect non-finite gradients before unscale/clip can change the
                # failure signature.
                self._raise_on_non_finite_training_state("after backward before unscale")

                self.scaler.unscale_(self.optim.optimizer)
                self._raise_on_non_finite_training_state(
                    "after unscale before gradient clipping"
                )
                ...
                if self.gradient_clipper is not None:
                    ...
                    self._raise_on_non_finite_training_state(
                        "after gradient clipping before optimizer step"
                    )
                ...
                self.scaler.update()
                self._raise_on_non_finite_training_state("after optimizer step")
    ...
    def _raise_on_non_finite_training_state(self, when: str) -> None:
        model = unwrap_ddp_if_wrapped(self.model)

        checks = (
            ("parameter", model.named_parameters(), None),
            (
                "gradient",
            ...
            (
                "gradient in language backbone",
                ((name, param.grad) for name, param in model.named_parameters()),
                FINITE_CHECK_PATTERNS,
            ),
        )
        for source, named_tensors, patterns in checks:
            summaries = _non_finite_tensor_summaries(
                named_tensors, patterns=patterns, limit=8
            )
            if summaries:
                detail = "\n  - ".join(summaries)
                step_state = ", ".join(
                    f"{phase}={step}" for phase, step in self.steps.items()
                )
                raise FloatingPointError(
                    f"Non-finite {source} detected {when} "
                    f"(epoch={self.epoch}, steps={{{step_state}}}, "
                    f"where={getattr(self, 'where', None)}). First bad tensors:\n"
                    f"  - {detail}"
                )
