...
class Sam3LossWrapper(torch.nn.Module):
    ...
    def _is_metric_key(key: str) -> bool:
        # Loss dicts mix optimization targets (loss_*) and monitoring metrics
        # (e.g. *_acc, *_f1, miou_*). The former are intentionally summed across
        # stages; the latter should be averaged across stages.
        return key != CORE_LOSS_KEY and not key.startswith("loss_")
    ...
    def forward(self, find_stages: SAM3Output, find_targets):
        with SAM3Output.iteration_mode(
            ...
            stage_outputs_list = list(find_stages)
            if find_stages.loss_stages is not None:
                stage_outputs_list = [
                    stage_outputs_list[i] for i in find_stages.loss_stages
                ]
                find_targets = [find_targets[i] for i in find_stages.loss_stages]
            assert len(stage_outputs_list) == len(find_targets)
            total_losses = {}
            metric_counts = {}
            for stage_outputs, stage_targets in zip(stage_outputs_list, find_targets):
                ...
                for outputs, targets in zip(stage_outputs, stage_targets):
                    ...
                    if self.normalize_by_stage_num:
                        cur_losses[CORE_LOSS_KEY] /= len(stage_outputs_list)
                    ...
                    for k, v in cur_losses.items():
                        if k not in total_losses:
                            ...
                            total_losses[k] += v
                        if self._is_metric_key(k):
                            metric_counts[k] = metric_counts.get(k, 0) + 1

        for key, count in metric_counts.items():
            if count > 0:
                total_losses[key] = total_losses[key] / count
