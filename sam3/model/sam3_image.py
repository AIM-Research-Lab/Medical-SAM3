...
class Sam3Image(torch.nn.Module):
    ...
    def _compute_matching(self, out, targets):
        ...
        indices = out["indices"]
        if len(indices[0]) > targets["num_boxes"].sum().item():
            logger.warning(
                "_compute_matching: %d matches but only %d target boxes "
                "(num_boxes=%s pred_logits.shape=%s)",
                len(indices[0]), targets["num_boxes"].sum().item(),
                targets["num_boxes"], out["pred_logits"].shape,
            )
